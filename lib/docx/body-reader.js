...
function BodyReader(options) {
    ...
    var xmlElementReaders = {
        ...
        "w:pPr": function(element) {
            return readParagraphStyle(element).map(function(style) {
                return {
                    ...
                    numbering: readNumberingProperties(element.firstOrEmpty("w:numPr"), numbering, styles.findNumberingStyleById(style.styleId)),
        ...
        "w:pict": function(element) {
            isInPict = true;
            let ret = readChildElements(element).toExtra();
            isInPict = false;
            return ret;
        },
...
function readNumberingProperties(element, numbering, numStyle) {
    var numId = element.firstOrEmpty("w:numId").attributes["w:val"];
    var level = element.firstOrEmpty("w:ilvl").attributes["w:val"];
    if (numId === undefined && numStyle !== undefined) {
        numId = numStyle.numId;
        if (numId !== undefined)
            if (level === undefined)
                level = 0;
    }
